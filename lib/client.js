	...
	factory: (require) => {
		...
		function providerSection(p) {
			...
			const foot =
				p.id === "volc-coding-plan"
					...
					: p.id === "deepseek"
					? `${p.name} · 官方余额 API · api.deepseek.com${stale}`
					: p.plan === "custom"
					? `${p.name} · 自定义套餐${stale}`
					: isOfficial
		...
		function QuotaSettingsForm() {
			...
			return h(
				...
				h(
					...
					h("span", { style: { fontSize: 12, fontWeight: 700, color: T.label } }, "DeepSeek 官方 API 余额"),
					h("input", {
						type: "password",
						value: dsKeyInput,
		...
		function SettingsSection() {
			return h(
				"div",
				{ style: { display: "flex", flexDirection: "column", gap: 10 } },
				h("span", { style: { fontSize: 12, color: T.secondary } }, "在侧栏底部展示各 Coding Plan 的剩余额度（OpenCode Go / 火山 CodingPlan / 自定义套餐）"),
				h(QuotaSettingsForm)
			);
		}
