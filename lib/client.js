window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function FooterBadge({ wide }) {
			...
			if (wide) {
				const title = PROVIDER_BLOCKS.filter(([id]) => !store.hidden.includes(id))
					.map(([id, label]) => {
						const p = prov(s, id);
						return p && p.windows && p.windows.length
							? `${label} 剩余: ${p.windows.map((w) => w.label + " " + fmtPct(remainingOf(w))).join(" · ")}`
							: null;
					})
					.filter(Boolean)
					.join("\n");
				return h(
					...
					PROVIDER_BLOCKS.filter(([id]) => !store.hidden.includes(id)).length === 0
						? h("span", { style: { color: T.secondary } }, "额度已全部隐藏（⚙ 开启）")
						: PROVIDER_BLOCKS.map(([id, title]) =>
								store.hidden.includes(id) ? null : providerBlock(s, title, prov(s, id))
						  )
				);
			}
			return h(
		...
		function providerSection(p) {
			...
			const foot =
				p.id === "codex"
					? `${p.name} · 官方套餐额度（rate_limits）${stale}`
		...
		function QuotaSettingsForm() {
			...
			const cx = settings && settings.codex;
			...
			return h(
				...
				h(
					"div",
					{ style: { display: "flex", flexDirection: "column", gap: 6, borderTop: "1px solid " + T.border, paddingTop: 8 } },
					h("span", { style: { fontSize: 12, fontWeight: 700, color: T.label } }, "Codex"),
					cx
						? h(
								"div",
								{ style: { display: "flex", flexDirection: "column", gap: 2, fontSize: 11, color: T.secondary } },
								cx.note === "no-data"
									? h("span", null, "未检测到本机 codex 会话额度快照")
									: h("span", null, "已检测：" + (cx.plan ? cx.plan.toUpperCase() + " · " : "") + (cx.windows || []).map((w) => w.label + " " + fmtPct(w.pct)).join(" · ") + "（数据 " + fmtAgo(cx.updatedAtMs) + "）"),
								h("span", null, "codex 额度随会话自动更新；如需重新登录，请在终端运行 codex login")
						  )
						: h("span", { style: { fontSize: 11, color: T.secondary } }, "加载中…"),
					h(
						"div",
						{ style: { display: "flex", gap: 6 } },
						h("button", { type: "button", onClick: () => { refresh(); setMsg("已刷新额度数据"); }, style: actionBtn(false) }, "立即刷新额度")
					)
				),
				msg
		...
		function SettingsSection() {
			return h(
				"div",
				{ style: { display: "flex", flexDirection: "column", gap: 10 } },
				h("span", { style: { fontSize: 12, color: T.secondary } }, "在侧栏底部展示各 Coding Plan 的剩余额度（Go / Codex / 火山）"),
				h(QuotaSettingsForm)
			);
		}
